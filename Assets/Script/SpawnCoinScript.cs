...
public class SpawnCoinScript : MonoBehaviour
{
   ...
   // Triggered when the object enters a collider trigger
    ...
    private void AddToLeaderboard(int score)
{
    // Only add the score if it surpasses an existing score or if the leaderboard isn't full
    if (leaderboardScores.Count < MaxLeaderboardSize || score > leaderboardScores[MaxLeaderboardSize - 1])
    {
        leaderboardScores.Add(score); // Add the current score to the leaderboard
        leaderboardScores.Sort((a, b) => b.CompareTo(a)); // Sort the scores in descending order
        if (leaderboardScores.Count > MaxLeaderboardSize)
        {
            leaderboardScores.RemoveAt(MaxLeaderboardSize); // Remove excess scores beyond the max size
        }
        SaveLeaderboard(); // Save the updated leaderboard in PlayerPrefs
    }
}
